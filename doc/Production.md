...
The production machine serves the back-end API and the front-end application.
It is started with pm2.
It runs behind a proxy nginx that maintains the https connection.
...
For security reason, you may want to restrict access to the application. Nginx can filter IP addresses.
See https://serverfault.com/questions/380642/nginx-how-to-redirect-users-with-certain-ip-to-special-page

Just modify the `gestion-stock.yoursite.com.conf` config as follow:

```
geo $bad_user {
    default 1;
    80.13.30.167/32 0;
    128.127.139.234/32 0;
    86.218.11.174/32 0;
}
